...
func (s *Staff) Node(ctx context.Context) (node *Node, err error) {
	node = &Node{
		ID:     s.ID,
		Type:   "Staff",
		Fields: make([]*Field, 6),
	...
	var buf []byte
	if buf, err = json.Marshal(s.UID); err != nil {
		return nil, err
	}
	node.Fields[0] = &Field{
		Type:  "string",
	...
	if buf, err = json.Marshal(s.Email); err != nil {
		return nil, err
	}
	node.Fields[1] = &Field{
		Type:  "string",
	...
	if buf, err = json.Marshal(s.Role); err != nil {
		return nil, err
	}
	node.Fields[3] = &Field{
		Type:  "staff.Role",
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case staff.Table:
		nodes, err := c.Staff.Query().
			Where(staff.IDIn(ids...)).
			CollectFields(ctx, "Staff").
			All(ctx)
		if err != nil {
			return nil, err
